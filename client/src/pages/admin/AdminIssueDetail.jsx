...
export default function AdminIssueDetail() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        const { data } = await api.get(`/admin/issues/${id}`, { headers });
        setIt(data);
      } catch (e) {
  ...
  if (!it) return (
    <div className="card bg-white p-20">
      ...
      <Link to="/all-bugs" className="btn btn-outline-secondary">Back</Link>
    </div>
  );
    ...
    <div className="card bg-white p-20 rounded-10 border border-white mb-4">
      ...
      <div className="row gy-3">
        <div className="col-md-4">
          ...
          <div className="mb-2">
            <strong>Reporter:</strong> {it.reporterId?.name} <span className="text-muted small">({it.reporterId?.email})</span>
          </div>
          <div className="mb-2">
            <strong>Assignee:</strong> {it.assigneeId ? (
              <>{it.assigneeId.name} <span className="text-muted small">({it.assigneeId.email})</span></>
            ) : "Unassigned"}
          </div>
          {it.pageUrl && (
            <div className="mb-2">
